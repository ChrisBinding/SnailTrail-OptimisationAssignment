...
// VERSION 2.2 - SWITCH statements re-ordered for Branch Prediction - also, FOR loop counters reversed
...
// BEST AVERAGE FRAME RATE FROM THIS VERSION: 349.6173
/*
...
int main()
{
	...
	void initialiseGame(char[][SIZEX], char[][SIZEX], int[], int[][2], int&, int&, bool&);
	...
	while ((newGame | 0x20) != QUIT)		// keep playing games
	{
		...
		for (int k(0); k < 20; k++)			// * For loop that allows the game to be automatically re-played 20 times -> allows all game results to be tested
		{
			...
			initialiseGame(garden, slimeTrail, snail, frogs, pellets, lettucesEaten, fullOfLettuce);
			...
			while (((key | 0x20) != QUIT) && snailStillAlive && !fullOfLettuce)	//user not bored, and snail not dead or full (main game loop)
			{
				...
				// *******************************************************************************
				// ************** Functions Inlined - To Improve Performance *********************
				// *******************************************************************************

				// ************** Analyse Key Function *******************************************
				// * RE-ORDERED FOR BRANCH PREDICTION
				...
				int targetY(snail[0] + move[0]);
				int targetX(snail[1] + move[1]);
				...
				case LETTUCE:															// increment lettuce count and win if snail is full
					...
					snail[0] += move[0];												//go in direction indicated by keyMove
					snail[1] += move[1];
					garden[snail[0]][snail[1]] = NO_LETTUCE;						// eat the lettuce
				...
				for (int f = NUM_FROGS - 1; f >= 0; f--)	// * Counter reversed for optimisation
				{
...
void initialiseGame(char garden[][SIZEX], char slimeTrail[][SIZEX],
...
{ //initialise garden & place snail somewhere
	...
	// ************** Set Snail Initial Coords Function *******************************************
	...
	// ************** Initialise Slime Trial Function *******************************************
	...
	garden[4][10] = LETTUCE;
	garden[7][2] = LETTUCE;
	garden[11][15] = LETTUCE;
	garden[8][15] = LETTUCE;

	// ************** Scatter Frogs Function *******************************************
