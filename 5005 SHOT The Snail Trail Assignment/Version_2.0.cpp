...
// VERSION 2.0 - lettucePatch array remvoved and lettuces put into garden array to clean up unneeded arrays
...
// BEST AVERAGE FRAME RATE FROM THIS VERSION: 192.9328
/*
...
int main()
{
	...
	void moveSnail(char[][SIZEX], char[][SIZEX], int[], int[], int&, string&);
	void moveFrogs(char[][SIZEX],int[], int[][2], string&);
		...
		for (int k(0); k < 20; k++)			// * For loop that allows the game to be automatically re-played 20 times -> allows all game results to be tested
		{
			...
			while (((key | 0x20) != QUIT) && snailStillAlive && !fullOfLettuce)	//user not bored, and snail not dead or full (main game loop)
			{
				...
				*/
				...
				moveSnail(garden, slimeTrail, snail, move, pellets, message);
				dissolveSlime(garden, slimeTrail);						// remove slime over time from garden
				placeSnail(garden, snail);								// move snail in garden
				moveFrogs(garden, snail, frogs, message);				// frogs attempt to home in on snail
...
void moveFrogs(char garden[][SIZEX], int snail[], int frogs[][2], string& msg)
{
...
void moveSnail(char garden[][SIZEX], char slimeTrail[][SIZEX], int snail[], int keyMove[], int& pellets, string& msg)
{
